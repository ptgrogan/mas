...
var ExplicitEuler = require('../sim/explicitEuler');

var _methods = [];
var METHOD_EXPLICIT_EULER = 'euler';
_methods[METHOD_EXPLICIT_EULER] = new ExplicitEuler();
...
function Simulator() {
	this.time = 0;
	this.method = METHOD_EXPLICIT_EULER;
...
Simulator.prototype.advance = function() {
	...
	var numEntities = this.entities.length; // cached to improve performance
	for(var i = 0; i < numEntities; i++) {
		this.entities[i].tick(this.timeStep, _methods[this.method]);
	}
	for(var i = 0; i < numEntities; i++) {
